/* TaskFlow Dashboard Notifications — no browser alerts/confirms */
(function(){
  'use strict';
  const STYLE_ID='tf-dashboard-notify-style';
  const TOAST_ID='tf-dashboard-notify-toast';
  const MODAL_ID='tf-dashboard-confirm';

  function ensureStyle(){
    if(document.getElementById(STYLE_ID)) return;
    const s=document.createElement('style'); s.id=STYLE_ID;
    s.textContent=`
      #${TOAST_ID}{position:fixed;left:24px;bottom:24px;z-index:10000;min-width:280px;max-width:420px;padding:14px 18px;border-radius:14px;background:rgba(23,26,35,.97);color:#fff;border:1px solid #10b981;box-shadow:0 14px 40px rgba(0,0,0,.35);display:flex;align-items:center;gap:10px;opacity:0;transform:translateY(18px);pointer-events:none;transition:.25s;font-size:14px;font-weight:700;direction:rtl}
      #${TOAST_ID}.show{opacity:1;transform:none}
  ...
  // Browser alerts become dashboard toasts.
  ...
  window.deleteMember=function(key){
    const m=(window.members||[]).find(x=>x.firebaseKey===key);
    const name=m?.name ? ` «${m.name}»` : '';
    dashboardConfirm(`هل أنت متأكد من حذف العضو${name} نهائياً؟`,function(){
      window.db.ref('members/'+key).remove().then(()=>success('تم حذف العضو بنجاح.')).catch(err=>success('تعذر حذف العضو: '+(err?.message||'خطأ غير معروف')));
    });
  };
  ...
  function install(){
    ...
    if(taskForm && !taskForm.dataset.tfNotifyWrapped){
      taskForm.onsubmit=function(e){
        ...
        const op=key?window.db.ref('tasks/'+key).update(data):window.db.ref('tasks').push(data);
    ...
    if(memberForm && !memberForm.dataset.tfNotifyWrapped){
      memberForm.onsubmit=function(e){
        e.preventDefault();
        const key=window.editingMemberKey||null;
        ...
        let op;if(key) op=window.db.ref('members/'+key).update(data);else{data.hiddenPages=[];op=window.db.ref('members').push(data);}
        Promise.resolve(op).then(()=>{window.closeMemberModal?.();success(key?'تم تعديل بيانات العضو بنجاح.':'تمت إضافة العضو بنجاح.');}).catch(err=>success('تعذر حفظ العضو: '+(err?.message||'خطأ غير معروف')));
      };
      memberForm.dataset.tfNotifyWrapped='1';
    }

    window.changeMyPassword=function(){
      const input=document.getElementById('newPassInput');const val=(input?.value||'').trim();
      if(!val){success('اكتب كلمة المرور الجديدة أولاً.');return;}
      window.db.ref('members/'+window.currentUserId).update({pass:val}).then(()=>{input.value='';success('تم تغيير كلمة المرور بنجاح.');}).catch(err=>success('تعذر تغيير كلمة المرور: '+(err?.message||'خطأ غير معروف')));
    };

    window.toggleBlockMember=function(key,st){
      const next=st==='disabled'?'active':'disabled';
      window.db.ref('members/'+key).update({status:next}).then(()=>success(next==='disabled'?'تم إيقاف العضو بنجاح.':'تم تفعيل العضو بنجاح.')).catch(err=>success('تعذر تحديث حالة العضو: '+(err?.message||'خطأ غير معروف')));
    };

    window.savePermissions=function(){
      const key=window.permMemberKey;if(!key)return;
      ...
      window.db.ref('members/'+key).update({hiddenPages:hidden}).then(()=>{window.closePermissionsModal?.();success('تم حفظ الصلاحيات بنجاح.');}).catch(err=>success('تعذر حفظ الصلاحيات: '+(err?.message||'خطأ غير معروف')));
    };
    return true;
  }

  function boot(){ensureStyle();if(install())return;setTimeout(install,300);setTimeout(install,1000);setTimeout(install,2000);}
